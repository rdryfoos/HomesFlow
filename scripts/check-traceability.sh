...
#   --matrix   Regenerate specs/001-mvp/coverage.md + coverage.svg (portfolio snapshot).
#   --json     Print the per-ID coverage dataset as JSON to stdout (on demand).
#   --canvas   Update the local Golden Thread Coverage canvas (Cursor; not in git).
#   --refresh  Gate 2 check + --matrix + optional canvas (after traceability changes).
#
# Checks (default mode):
...
emit_matrix() {
...

  {
    ...
    echo "Portfolio snapshot for cold readers; CI enforces the golden thread via the gate script, not file freshness."
    echo "Source of truth: \`HomesFlow.prd.md\` registry × \`tasks.md\` × \`@covers\` annotations × test names."
    echo
    echo "## Summary"
    echo
    echo "| Metric | Count |"
